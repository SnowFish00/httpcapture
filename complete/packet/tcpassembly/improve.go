...
type StreamFactory interface {
	...
	//test
	New(netFlow, tcpFlow gopacket.Flow, repacket gopacket.Packet) Stream
}
...
func (p *StreamPool) getConnection(k key, end bool, ts time.Time, repacket gopacket.Packet) *connection {
	p.mu.RLock()
	conn := p.conns[k]
	p.mu.RUnlock()
	if end || conn != nil {
	...
	//test
	s := p.factory.New(k[0], k[1], repacket)
	p.mu.Lock()
...
// test
func (a *Assembler) Assemble(netFlow gopacket.Flow, t *layers.TCP, repacket gopacket.Packet) {
	a.AssembleWithTimestamp(netFlow, t, time.Now(), repacket)
}
...
//
// test
func (a *Assembler) AssembleWithTimestamp(netFlow gopacket.Flow, t *layers.TCP, timestamp time.Time, repacket gopacket.Packet) {
	...
	for {
		conn = a.connPool.getConnection(
			key, !t.SYN && len(t.LayerPayload()) == 0, timestamp, repacket)
		if conn == nil {
			if *debugLog {
